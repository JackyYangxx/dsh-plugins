...
- **Peer ranges are pinned to the built-against rc.8 line.** The `@deepseek-ai/dsh-*` peers use `^0.1.0-rc.8` (matching devDependencies), which by semver prerelease rules does NOT accept the host CLI closure's `0.1.1-rc.2` — all peers are optional and profiles install with `autoInstallPeers: false`, so the plugin resolves its own rc.8 copies and the dual-instance layout is what actually runs (verified end to end). Revisit the range if a future host line should be officially supported.
